...
Plot coordinates of max occurrence likelihood 
...
# Syntenty plot of ancestral AFP array... this is just a test of the code
# synt_plot is not formatted
...
synt_plot # synt_plot is not formatted... just a test
...
Instead of all-to-all links between homologous regions of synt plot, use neighbor joining

```{r}

# Create pairwise links between nearest neighbors only
pairwise_links <- df_long_synt_filt %>%
  arrange(block_id, fish_num) %>%
  group_by(block_id) %>%
  mutate(next_fish_num = lead(fish_num),
         next_pos = lead(pos_adjstd)) %>%
  filter(!is.na(next_fish_num) & abs(fish_num - next_fish_num) == 1) %>%
  ungroup()

# Convert fish_num to numeric if not already
df_long_synt_filt$fish_num <- as.numeric(df_long_synt_filt$fish_num)

# Plot
synt_synt_fix <- ggplot(data = df_long_synt_filt, 
       aes(y = fish_num,
           x = pos_adjstd)) +
  geom_segment(data = pairwise_links,
               aes(x = pos_adjstd, xend = next_pos,
                   y = fish_num, yend = next_fish_num),
               color = "gray80", alpha = 0.25, linewidth = 0.25) +
  geom_rect(aes(xmin = min(filter(df_long_synt_filt, species == "seq0")$leftend_adj), 
                xmax = max(filter(df_long_synt_filt, species == "seq0")$rightend_adj),
           ymin = 2 - 0.035, 
           ymax = 2 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_synt_filt, species == "seq1")$leftend_adj), 
                xmax = max(filter(df_long_synt_filt, species == "seq1")$rightend_adj),
           ymin = 1 - 0.035, 
           ymax = 1 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_synt_filt, species == "seq2")$leftend_adj), 
                xmax = max(filter(df_long_synt_filt, species == "seq2")$rightend_adj),
           ymin = 5 - 0.035, 
           ymax = 5 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_synt_filt, species == "seq3")$leftend_adj), 
                xmax = max(filter(df_long_synt_filt, species == "seq3")$rightend_adj),
           ymin = 4 - 0.035, 
           ymax = 4 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_synt_filt, species == "seq4")$leftend_adj), 
                xmax = max(filter(df_long_synt_filt, species == "seq4")$rightend_adj),
           ymin = 3 - 0.035, 
           ymax = 3 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_synt_filt, species == "seq5")$leftend_adj), 
                xmax = max(filter(df_long_synt_filt, species == "seq5")$rightend_adj),
           ymin = 3 - 0.035, 
           ymax = 3 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_synt_filt, species == "seq6")$leftend_adj), 
                xmax = max(filter(df_long_synt_filt, species == "seq6")$rightend_adj),
           ymin = 6 - 0.035, 
           ymax = 6 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = pos_adjstd-0.0002, xmax = pos_adjstd+0.0002, group = as.factor(block_id), 
                fill = as.factor(block_id), 
  ...
  labels = c("A. minor", "A. lupus", "O. amberensis",
             "L. platyrhina", "L. maculatus", "P. gunnelus")
  ) +
  theme(legend.position = "none",
      axis.title.y = element_blank(),
...
synt_plot_filt_fix <- ggplot(data = df_long_filt, 
       aes(y = fish_num, x = pos_adjstd)) +
  geom_rect(aes(xmin = min(filter(df_long_filt, species == "seq0")$leftend_adj), 
                xmax = max(filter(df_long_filt, species == "seq0")$rightend_adj),
                ymin = 1 - 0.035, ymax = 1 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_filt, species == "seq1")$leftend_adj), 
                xmax = max(filter(df_long_filt, species == "seq1")$rightend_adj),
                ymin = 2 - 0.035, ymax = 2 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_filt, species == "seq2")$leftend_adj), 
                xmax = max(filter(df_long_filt, species == "seq2")$rightend_adj),
                ymin = 2 - 0.035, ymax = 2 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_filt, species == "seq3")$leftend_adj), 
                xmax = max(filter(df_long_filt, species == "seq3")$rightend_adj),
                ymin = 5 - 0.035, ymax = 5 + 0.035), fill = "gray80") +
  geom_rect(aes(xmin = min(filter(df_long_filt, species == "seq4")$leftend_adj), 
                xmax = max(filter(df_long_filt, species == "seq4")$rightend_adj),
                ymin = 5 - 0.035, ymax = 5 + 0.035), fill = "gray80") +
  geom_line(aes(x = pos_adjstd, 
                y = fish_num, group = as.factor(block_id)),
            stat = "identity", color = "gray80", 
            alpha = 0.25, size = 0.25, orientation = "x") +
  geom_rect(aes(xmin = leftend_adj, xmax = rightend_adj, group = as.factor(block_id), 
                fill = as.factor(block_id), 
                ymin = fish_num - 0.15, ymax = fish_num + 0.15)) +
  geom_point(data = point_df,
             aes(x = pos_adjstd, y = fish_num), color = "goldenrod4", size = 2.5) +
  geom_point(data = point_df,
             aes(x = pos_adjstd, y = fish_num), color = "goldenrod1", size = 1.5) +
  scale_y_continuous(
    breaks = c(1, 2, 3, 4, 5, 6),
    labels = c("A. lupus", "A. minor", "O. amberensis", 
               "L. platyrhina", "L. maculatus", "P. gunnelus")
  ) +
  theme_classic(base_size = 20) +
  theme(legend.position = "none",
...
# Create synteny panel
synt_synt_fix <- synt_synt_fix + theme(plot.margin = margin(.1, .1, .1, .1, "cm"))
synt_plot_filt_fix <- synt_plot_filt_fix + theme(plot.margin = margin(.1, .1, .1, .1, "cm"))

Fig_3B_neighbor <- ggarrange(synt_synt_fix, synt_plot_filt_fix,
                   labels = c("", ""),
                   widths = c(1, .95),
                   ncol = 2, nrow = 1, align = "h")

# Export 3B neighbor
png("~/Documents/GitHub/Long_AFP_04082024/Figures/Fig_3B_neighbor.png", units = "in", width = 12,
    height = 2.66,
    res = 600)
