...
# Verify the class of Alupus_AFPsynt_gr
class(Alupus_AFPsynt_gr) # This should return "GRanges"

# Extract  chromosome names
chr <- as.character(unique(seqnames(Alupus_AFPsynt_gr)))

# Extract genome information. This typically refers to the reference name or assembly ID
# Read in gff file for species if this doesn't work
gen <- genome(Alupus_AFPsynt_gr)

# Create an AnnotationTrack object for visualizations

# disabled: options(ucscChromosomeNames=FALSE)
atrack <- AnnotationTrack(Alupus_AFPsynt_gr, name = "Alupus_AFPsynt") 

# Create a GenomeAxisTrack for displaying a genome axis
...
# Create annotation tracks for the GRanges objects
...
Aminor_TEtransB_f <- filter(Aminor_TEtransA_gr,
                          start >= min(as.data.frame(Aminor_AFPtransB_gr)$start) &
                            end <= max(as.data.frame(Aminor_AFPtransB_gr)$end)) ### Empty because there is only one AFP - use proximity to object
...
Lmac_TEtransB_f <- filter(Aminor_TEtransA_gr,
                          start >= min(as.data.frame(Lmac_AFPtransB_gr)$start) &
                            end <= max(as.data.frame(Lmac_AFPtransB_gr)$end))
